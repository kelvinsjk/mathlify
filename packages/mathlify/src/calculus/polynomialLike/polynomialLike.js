	...
	constructor(coeffsPowerArr, options) {
		...
		coeffsPowerArr.forEach(([coeff, power]) => {
			...
			if (coeffsMap.has(powerFrac)) {
				...
				coeffsMap.set(power, currentCoeff.plus(coeffFrac));
			} else {
				coeffsMap.set(power, coeffFrac);
			}
		});
		/** @type {Term[]} */
		const terms = [];
			...
			if (power.is.zero()) {
				...
				if (power.den === 2) {
					...
					terms.push(
						new Term(
							coeff.sign(),
							`\\frac{${coeff.abs().num}}{${new Term(coeff.den, sqrtString)}}`
						)
					);
				} else {
					terms.push(
						new Term(
							coeff.sign(),
							`\\frac{${coeff.abs().num}}{${new Term(coeff.den, [
								variable,
								power.abs(),
							])}}`
						)
