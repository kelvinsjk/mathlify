...
import { Polynomial, Fraction, numberToFraction } from "../../core/index.js";
import { RationalTerm, longDivide } from "../../algebra/index.js";
import { productRulePoly } from "../utils/product-rule.js";
...
export class RationalFn extends RationalTerm {
  ...
  constructor(numerator, denominator = 1) {
    ...
    let denPow = 1;
    if (typeof denominator === "number" || denominator instanceof Fraction) {
      denominator = new Polynomial([denominator]);
    } else if (Array.isArray(denominator)) {
      [denominator, denPow] = denominator;
    }
    ...
    super(numerator, [denominator, denPow], options);
    this.num = numerator;
    this.numFn = numerator;
    this.denFn = denominator;
    this.denPow = numberToFraction(denPow);
   ...
   */
  differentiate() {
    if (this.numFn.is.constant()) {
      // d/dx (k (gx)^-n) = -nk g' (gx)^(-n-1)
      return new RationalFn(
        this.denFn
          .differentiate()
          .times(this.denPow)
          .times(this.num.negative())
          .times(this.coeff),
        [this.denFn, this.denPow.plus(1)]
      );
    }
    const num = this.numFn.times(this.coeff);
    const numResult = productRulePoly(num, [this.denFn, this.denPow], {
      quotientMode: true,
      polyReturn: true,
    });
    // TODO: handle case where canceling powers is possible
    return new RationalFn(numResult, [this.denFn, this.denPow.times(2)]);
  }

  // TODO: handle denPow for all methods below
  /**
   * @overload
   * multiplication
  ...
  times(x) {
    if (typeof x === "number" || x instanceof Fraction) {
      return new RationalFn(this.numFn.times(x).times(this.coeff), [
        this.denFn,
        this.denPow,
      ]);
    }
    if (x instanceof RationalFn) {
      return new RationalFn(
        ...
        `${this.denFn.minus(x.denFn)}` === "0"
          ? [this.denFn, this.denPow.plus(x.denPow)]
          : this.denFn.pow(this.denPow).times(x.denFn.pow(x.denPow))
      );
    }
   ...
   */
  subInFraction(x) {
    return this.numFn.subIn(x).divide(this.denFn.subIn(x).pow(this.denPow));
  }
   ...
   */
  replaceXWith(x) {
    return new RationalFn(this.numFn.replaceXWith(x), [
      this.denFn.replaceXWith(x),
      this.denPow,
    ]);
  }
