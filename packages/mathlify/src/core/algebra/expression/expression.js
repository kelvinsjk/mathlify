 ...
 * @property {Map<string,"auto"|"never"|"always">} termFractionalDisplayMap - key: term signature, value: the fractionalDisplayMode of the term
...
export class Expression {
	...
	constructor(...terms) {
		...
		/** @type {Map<string,"auto"|"always"|"never">} */
		...
		terms.forEach((term) => {
			if (typeof term === 'number' || term instanceof Fraction) {
				...
				if (!termFractionalDisplayMap.has(term)) {
					termFractionalDisplayMap.set(term, 'auto');
				}
				...
				if (typeof term.term === 'number' || term.term instanceof Fraction) {
					...
					if (!termFractionalDisplayMap.has(term.term)) {
						termFractionalDisplayMap.set(term.term, 'auto');
					}
				} else {
		...
		termCoeffMap.forEach((coeff, variable) => {
			if (coeff.is.not.zero()) {
				...
				newTerm.setDisplayMode(
					this.termFractionalDisplayMap.get(variable) ?? 'auto'
				);
