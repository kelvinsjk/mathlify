...
export class Term {
  ...
  times(x) {
    ...
    const newTerm = powerMapToTerm(newPowerMap, this.coeff.times(x.coeff));
    if (this.fractionalDisplayMode || x.fractionalDisplayMode) {
   ...
   */
  reciprocal(options = { fractionalDisplayMode: false }) {
    const newPowerMap = new Map(this.powerMap);
    ...
    if (this.fractionalDisplayMode || options.fractionalDisplayMode) {
      newTerm.setFractionalDisplay();
    }
    return newTerm;
  }
