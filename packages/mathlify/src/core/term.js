...
export class Term {
  ...
  constructor(...args) {
    ...
    this.variables = Array.from(this.powerMap.keys());
  }
    ...
    if (this.is.rational() && x instanceof Term) {
      ...
      if (typeof x === "number" || x instanceof Fraction) {
        ...
        if (x instanceof Term) {
          ...
          throw new Error(
            "Expression multiplication not available to term class. consider reordering the multiplication order"
          );
        }
      }
    }
  ...
  divide(x, options) {
    if (
      ...
      throw new Error("division by expression not available in term class");
    }
    if (x.coeff.is.zero()) {
