import { SquareRoot, Term, Fraction, Expression } from "../index";
...
test("sqrt and expressions", () => {
  const root2Minus1 = new Expression(root2, -1);
  expect(root2Minus1.toTex()).to.eq("\\sqrt{2} - 1");
  const square = root2Minus1.times(root2Minus1);
  expect(square.toTex()).to.eq("3 - 2 \\sqrt{2}");
  expect(square.terms[1].type).to.eq("sqrt");
});
