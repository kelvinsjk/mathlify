import { Fraction, Unknown, BasicTerm, Term, Expression } from '../core/index';
...
export class uxVector {
	 ...
	 * @param vector the a in k a
	 */
	constructor(vector: string, coeff: Fraction | number | string | Unknown | Term = 1) {
		// bold face if not already
		vector = vector.slice(0, 7) === '\\mathbf' ? vector : `\\mathbf{${vector}}`;
		if (coeff instanceof Term) {
			this.coeff = coeff.clone();
		} else {
			this.coeff = coeff instanceof Unknown ? new Term(1, coeff) : new Term(coeff);
		}
		this.vector = vector;
	}
