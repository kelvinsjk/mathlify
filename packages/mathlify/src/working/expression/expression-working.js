...
export class ExpressionWorking {
  ...
  expand(options) {
    ...
    if (
      ...
      this.exp.terms[0] instanceof ExpansionTerm
    ) {
      this.exp = this.exp.terms[0].expand();
    } else if (
      this.exp instanceof UnsimplifiedExpression &&
      this.exp.terms.length === 1 &&
      this.exp.terms[0].term instanceof ExpansionTerm
    ) {
      this.exp = this.exp.terms[0].term.expand();
    }
    this.expArray.push(this.exp);
    return this;
  }
  ...
  changeOrder(args, options) {
    ...
    const newTerm = [];
    args.forEach((arg) => {
      newTerm.push(terms[arg]);
    });
    newTerm.push(...terms.filter((_, i) => !args.includes(i)));
    this.exp = new Expression(...newTerm);
    this.expArray.push(this.exp);
    return this;
  }
