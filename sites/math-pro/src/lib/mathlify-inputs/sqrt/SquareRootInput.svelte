<script context="module" lang="ts">
	...
	import { keyboardOptions, sqrtLayer } from './keyboardLayout';
	import { isValidSqrt } from './parser';
	import type { SquareRoot } from 'mathlify';
...
<script lang="ts">
	...
	export let value: SquareRoot = undefined;
	...
	onMount(async () => {
		...
		mathliveElement.setOptions({
			...
			onFocus: focusScroll,
			customVirtualKeyboardLayers: sqrtLayer,
		});
	...
	function updateValue(mathfield) {
		...
		const [valid, values] = isValidSqrt(mathfieldValue);
		if (valid) {
			({ type, sqrt: value, simplified } = values);
			invalid = false;
			if (mathliveContainer) {
...
<style>
	.mathfield-input {
		...
		font-weight: 700;
		color: black;
		border: 3px solid hsl(224, 41%, 75%);
