...
function qn6(variables?: {
	...
	const poly = new Polynomial([m2a], { unknown: 'r', degree: n3a + n2a })
		.minus(new Polynomial([m3a + m2a], { unknown: 'r', degree: n2a }))
		.plus(m3a);
	...
	// typeset qn
	const body = `An arithmetic seroes has first term ${math(`a`)}
		and common difference ${math(`d,`)} where ${math(`a`)}
		and ${math(`d`)} are non-zero.
		A geometric series has first term ${math(`b`)} and common ratio
		${math(`r,`)} where ${math(`b`)} and ${math(`r`)} are non-zero.
		<div class="top-margin">
			It is given that the ${math(`${m1 + 1}${suffix(m1 + 1)},`)}
			${math(`${m2}${suffix(m2)}`)}
			and ${math(`${m3}${suffix(m3)}`)} term of the arithmetic series are
			equal to the ${math(`${n1 + 1}${suffix(n1 + 1)},`)}
			${math(`${n2}${suffix(n2)}`)}
			and ${math(`${n3}${suffix(n3)}`)} term of the geometric series respectively.
		</div>
	`;
	const partI = `Show that ${math(`r`)} satisfies the equation ${math(`${poly}.`)}
		<div class="top-margin">
			Given that ${math(`|r|<1,`)} solve this equation, giving your answer correct to 2 decimal places.
		</div>
	`;
	const partII = `By using this value of ${math(`r,`)} find, in terms of
		${math(`b`)} and ${math(`n,`)} the sum of the terms of the geometric series after,
		but not including, the ${math(`n\\textrm{th}`)} term, simplifying your answer.`;

	// solution working
	const coeff = 1 / (1 - r);

	// answer
	const ansI = math(`r = ${r.toFixed(2)}.`);
	const ansII = `${math(`${coeff.toPrecision(3)}\\left( ${r.toFixed(2)} \\right)^n b.`)}`;
