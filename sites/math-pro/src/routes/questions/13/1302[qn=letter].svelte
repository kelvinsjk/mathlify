<script lang="ts">
	...
	import {JSONParse, Fraction, type Term, type Expression, Vector, SquareRoot, getRandomFrac, getRandomInt, heads} from 'mathlify';
	import {checkAnswer} from './_logic/checkAnswer';
	import {qnProgress, defaultQnProgress} from '$lib/progress/stores';
	import {toast, SvelteToast} from '@zerodevx/svelte-toast';

	import {page, navigating, getStores} from '$app/stores';	
	import {scale} from 'svelte/transition';

	// edit this
	import {qnLogics} from './_logic/1302';
	const regex = /^1302[a-f]$/;
	// edit this
	...
	let submitted = false;
	let correct: boolean;
  let attempts: (Fraction | SquareRoot | number | Vector | number[])[];
	$: if (checked) {
		disabled = true;
