<script lang="ts">
	...
	import {qnLogics} from './_logic/1413';
	const regex = /^1413[(a-b)]$/;
	const sliceLocation = 4;
	// edit this
	
	let pageID = $page.url.href.slice($page.url.href.length-sliceLocation-1)
	let qn = pageID[0] === '/' ? pageID.slice(1) : pageID;
	// edit this
	let title = `Complex Numbers Qn ${qn}`
	// edit this
	let qnName = `${qn.slice(sliceLocation)}`;
	let qnLogic = qnLogics[qnName];
	
	export let varsPrimitive = {};
	...
	$: if (regex.test($page.url.href.slice($page.url.href.length-sliceLocation-1))) {
		...
		qnName = `${qn.slice(sliceLocation)}`; // edit this
