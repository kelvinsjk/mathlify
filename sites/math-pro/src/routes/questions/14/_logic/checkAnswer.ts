...
import { Fraction, SquareRoot, Complex } from 'mathlify';

export function checkAnswer(
	answers: Answer[],
	attempts: (Fraction | SquareRoot | number | Complex | number[])[],
): boolean {
	return answers.every((answer, i) => {
		...
		} else if (answer.value instanceof Complex) {
			if (attempt instanceof Complex) {
				return answer.value.isEqualTo(attempt);
			}
			throw new Error(`Expected ${attempt} to be Complex`);
