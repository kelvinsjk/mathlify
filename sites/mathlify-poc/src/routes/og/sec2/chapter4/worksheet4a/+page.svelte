<script lang="ts">
	...
	(() => {
		...
		const termsArray = [
			...
			[[5,'m'], [2, 'n']],
			[[3,'m'],[10, 'n']],
			[[2,'p'], new Fraction(1,4)],
			[[new Fraction(1,4),'q'], 2],
			[[8, 'p'], [new Fraction(3,2),'q']],
			[[new Fraction(2,5),'p'], [10, 'q']],
			[['x','y'],4],
			[6,[5,'x','y']],
			[[new Fraction(1,6),'x','y'],[3,'z']],
			[[new Fraction(3,8),'x'],[new Fraction(4,5),'y','z']]
		];
		termsArray.forEach((terms, i) => {
			let qn: string|ExpansionTerm;
			let ans: Expression;
			if (i===7){
				const exp1 = new ExpansionTerm(new Expression(terms[0],terms[1]));
				const exp2 = new ExpansionTerm(new Expression(terms[1],terms[0]))
				qn = `(${exp1}) (${exp2})`
				ans = exp1.expand().square();
