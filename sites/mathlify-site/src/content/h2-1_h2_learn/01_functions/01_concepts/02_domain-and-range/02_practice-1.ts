...
export interface IntervalOneSided {
	type: 'left' | 'right';
	inclusive: boolean;
	x: number;
}
...
export const practiceTitle = `range of function`;
...
export function generateQn(state: State): PracticeQuestion {
	...
	if (state.unknownConstants) {
		if (state1.fnType === 'frac') {
			qn = mathlifier`Find the range of the function
...
$${fnString}.`;
	...
	const ans =
		(state1.fnType === 'abs' || state1.fnType === 'improper') && state1.unknownConstants
			? mathlifier`$${'R_f'} = ${generateRange(state1, exp).join(' \\cup ')}.
