...
export function inverseRelationshipSolver(
	...
	let soln = mathlifier`The graph of ${'{y = f^{-1}(x)}'}
...
can be found by solving`;
	...
	if (fnType === 'quadratic') {
		...
		roots.push(root);
		soln += mathlifier`$${'gather*'}f(x) = x \\\\ ${working}
...
`;
		...
		soln += mathlifier`$${'gather*'}f(x) = x \\\\ ${working} ${extraQED}
...
`;
	...
	const ans = mathlifier`${'x='}${answers.join(', ')}.`;
