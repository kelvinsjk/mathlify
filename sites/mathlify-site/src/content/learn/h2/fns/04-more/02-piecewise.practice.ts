...
import { generateState as generateState1 } from '../01-concepts/02-functions.practice';
...
import { Interval, intervalBuilder } from '../intervals';
...
function generatePieceWiseFn(state: State): [string, Expression, Expression] {
	...
	const string = otherwise
		? mathlify`$${{}} f(x) = \\begin{cases} ${gExp} \\quad &\\text{for } ${intervalToInequality(intervalG)},
...
function generateAns(state: State, g: Expression, h: Expression): [string, Expression] {
	...
	if (intG.includes(x)) {
		...
		working.simplify();
		const ans = mathlify`$${'align*'} f(${x}) ${working}.`;
	...
	return [mathlify`$${{}}f(x)=0.`, new Expression(0)];
}
