...
import { sqrtInverse } from '$content/learn/h2/fns/02-inverse/03-formula.practice';
...
// aii
{
	...
	const soln1 = mathlifyQED`$${'gather*'} ff(x) = x \\\\ ${working}`;
	const x2 = cubicRoot(cubic);
	const x1 = bisection(cubic.fn, 0, 1);
	const x3 = bisection(cubic.fn, 2, 3);

	const soln2 = mathlifyQED`Solving with a GC,
...
$${{}} x = ${x3.toPrecision(3)}`;
	const soln3 = mathlifyQED`From ${{}}{ff(x)=x},
we can apply ${'f^{-1}'}
on both sides
...
obtained satisfies the equation ${{}} {f(x)=f^{-1}(x)}`;
	const ans = mathlify`${{}} x = ${x3.toPrecision(3)}.`;
...
// b
{
	...
	const soln = mathlify`$${'align*'} g(2) &= ${evenCase.subIn({ n: 2 })}
\\\\ &= 2 + ${oddCase.subIn({ n: 1 })}
\\\\ &= 3 + 1
\\\\ &= 4
...
${{}} g(12) = 9.`;
	...
	// bii
	const soln2 = mathlifyQED`$${'align*'} g(8) &= ${evenCase.subIn({ n: 8 })}
\\\\ &= 2 + 6
\\\\ &= 8
