<script context="module" lang="ts">
  ...
  type TwoSidedInterval = {type: "two", x1: number|[number,number], x2: number|[number,number], inclusive: [boolean, boolean] };
  ...
  */
  export function xToNumber(x: number|[number,number], options:{xMin: number, xMax: number, xLeft: number, xRight: number}): number {
...
<script lang="ts">
  const {xMin = -10, xMax = 10, intervals=[
  ...
  const viewBox = `${x1-xWidth/20} ${-yUnit*3} ${xWidth*1.2} ${height*.9}`;
  const color = "black";
  const strokeWidth = 2;
</script>
...
<svg xmlns="http://www.w3.org/2000/svg" {viewBox} class="number-line">
  <defs>
    ...
    <marker id="open" markerWidth="5" markerHeight="5" refX="1" refY="1">
      <circle cx="1" cy="1" r="0.75" fill="white" markerUnits="strokeWidth" style={`stroke:${color}; stroke-width:${strokeWidth/4}`} />
    </marker>
  ...
  <!--axis-->
  <line {x1} {x2} {y1} {y2} style={`stroke:${color};stroke-width:${strokeWidth}`} marker-end="url(#arrow)" />
  <!--x label-->
  ...
  {#each intervals as interval}
    ...
    <line x1={scale(x)} x2={scale(x)} y1={-yUnit*.8} y2={yUnit*.8} style={`stroke:${color};stroke-width:${strokeWidth/2}`} />
    {#if interval.type==='two'}
    {@const x1 = scale(interval.x2)}
    <line {x1} x2={x1} y1={-yUnit*.8} y2={yUnit*.8} style={`stroke:${color};stroke-width:${strokeWidth/2}`} />
    {/if}
    <!--marker label-->
    ...
    <!--interval-->
    {#if interval.type==='two'}
    <line x1={scale(interval.x1)} x2={scale(interval.x2)} y1={-yUnit*2} y2={-yUnit*2} style={`stroke:${color};stroke-width:${strokeWidth}`}
      marker-start={`url(#${interval.inclusive[0] ? 'closed' : 'open'})`}
      marker-end={`url(#${interval.inclusive[1] ? 'closed' : 'open'})`}
    />
    {:else}
    <line x2={interval.type==='left' ? xLeft : x2} x1={scale(interval.x)} y1={-yUnit*2} y2={-yUnit*2} style={`stroke:${color};stroke-width:${strokeWidth}`}
