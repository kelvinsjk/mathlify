...
export function generateQn(state: State) {
	...
	const qn = renderHTML(
		`Write down the set described by the following ${inequalityText} in interval notation.`,
	);
	const ans = renderHTML(`$$\`${generateAns(state)}\``);
	const inequalityOrInterval: string | Interval[] = state.isInequality
		? renderHTML(`$$\`${generateInequality(state)}\``)
