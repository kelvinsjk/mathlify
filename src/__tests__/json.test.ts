...
json('single item', () => {
  assert.throws(()=>JSONParse('{"type":"impossible","args":[1]}'));
	...
	assert.is(oneSixthX1Json, '{"type":"expression","args":[{"type":"term","args":[{"type":"fraction","args":[1,6]},"x"]},{"type":"term","args":[{"type":"fraction","args":[1,1]},""]}]}');
	assert.is(oneSixthX1Parsed.toString(), '\\frac{1}{6} x + 1');
	assert.is(oneSixthX2Json, '{"type":"polynomial","args":[[{"type":"fraction","args":[1,6]},{"type":"fraction","args":[2,1]}],{"ascending":false,"degree":1,"variableAtom":"x"}]}');
