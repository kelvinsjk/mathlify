...
const vectorClasses = suite('vector classes');

vectorClasses('simplify and expand', () => {
  ...
  assert.is(negativeTwoIFourJSixKSimplified.toIJKString(), '2 \\left( - \\mathbf{i} + 2 \\mathbf{j} + 3 \\mathbf{k} \\right)');
  assert.is(negativeTwoIFourJSixKStretchable.toIJKString(), '- \\mathbf{i} + 2 \\mathbf{j} + 3 \\mathbf{k}');


});

vectorClasses('toString', () => {
...
vectorClasses('arithmetic', () => {
  ...
  assert.is(twoIMinusFourK.isEqualTo(twoIMinusFourKV2), true);
	...
	assert.equal([twoIMinusJMinusFiveK.x.valueOf(), twoIMinusJMinusFiveK.y.valueOf(), twoIMinusJMinusFiveK.z.valueOf()], [2, -1, -5]);
	assert.equal([twoIMinusJMinusFiveKV2.x.valueOf(), twoIMinusJMinusFiveKV2.y.valueOf(), twoIMinusJMinusFiveKV2.z.valueOf()], [2, -1, -5]);
	
	const minusIPlusTwoK = iMinusTwoK.negative({multiplyIntoCoeff: true});
	assert.equal([minusIPlusTwoK.coeff.valueOf(), minusIPlusTwoK.x.valueOf(), minusIPlusTwoK.y.valueOf(), minusIPlusTwoK.z.valueOf()], [-1, 1, 0, -2]);
