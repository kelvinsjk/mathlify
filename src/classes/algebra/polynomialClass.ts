...
*/
	constructor(coefficients: (number | Fraction)[], options?: Partial<PolynomialOptions>) {
		const polyOptions: PolynomialOptions = { ascending: false, degree: coefficients.length - 1, variableAtom: 'x', ...options };
		...
		if (!polyOptions.ascending) { // reverse coefficient array if descending
		...
		while (coeffs[coeffs.length - 1].isEqualTo(0) && coeffs.length > 1) { // non-zero leading coefficient (unless constant term)
